...
def evolucion_mario(n_Entrada,n_Oculta1,n_Oculta2,n_Salida,pesos_predefinidos,obs):
    ...
    for step in range(1000):
        ...
        #print("resultado_forward:", resultado_forward)

        accion_elegida = int(np.argmax(resultado_forward))
        #print("accion_elegida:",accion_elegida)
        #print(COMPLEX_MOVEMENT[accion_elegida])
...
for i in range(0,numero_generaciones):
    ...
    # fila_original = mejor_generacion[i]
    #arrays = fila_original.values

    array = np.array(mejor_generacion.iloc[i])
    pesos_predefinidos = lista_pesos_a_dicc_pesos(array)
    resul, obs = evolucion_mario(n_Entrada,n_Oculta1,n_Oculta2,n_Salida, pesos_predefinidos,obs)
...
for j in range(0, gen):
    ...
    print(mejor_generacion)
    print(mejores_recompensas)
    print("\n"*5)
